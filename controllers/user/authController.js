...
const verifyOtp = async (req, res) => {
  try {
    ...
    if (otp.trim() === req.session.userOtp.trim()) {
      ...
      const refferedUser = await User.findOne({
        referalCode: req.session.userData.referralCode,
      });
...
const resendOtp = async (req, res) => {
  try {
    ...
    if (emailSend) {
      ...
      res.status(500).jsom({
        success: false,
        message: "filed to resend OTP, please try again",
      });
      return res
        .status(200)
        .json({ success: true, message: "OTP resent successfully" });
...
const verifyForgot = async (req, res) => {
  try {
    ...
    console.log("otp:", otp);
    if (
      otp.trim() === req.session.userOtp.trim() &&
