...
const deleteAddress = async (req, res) => {
  ...
  try {
    const deleteAddresses = await Address.updateOne(
      { "address._id": addressId },
...
const addOrder = async (req, res) => {
  ...
  try {
    ...
    const user = await User.findOne({ _id: req.session.user }).session(session);
    const selectedAddress = await Address.findOne(
      { userId: user, "address._id": selectedAddressId },
      { "address.$": 1 }
    ).session(session);

    const cart = await Cart.findOne({ userId: user }).session(session);
    ...
    // --- Step 2: Handle wallet deduction if payment method is wallet ---
    ...
    // --- Step 4: Update coupon (if used) ---
    if (couponCode) {
      await Coupon.updateOne(
        { code: couponCode },
        { $push: { userId: req.session.user } },
        { session }
      );
    }
    ...
    // --- Step 6: Add transaction entry (wallet) ---
    if (paymentMethod === "wallet") {
      user.transactions.push({
        type: "debit",
        amount: finalPrice,
        description: `Amount debited for order ${order._id}`,
      });
...
const verifyPayment = async (req, res) => {
  try {
    ...
    if (!order || order.paymentMethod !== "razorpay") {
      return res
        .status(400)
        .json({ success: false, message: "Invalid order or payment method." });
    }
...
const cancelOrder = async (req, res) => {
  try {
    ...
    if (order) {
      ...
      res.json({
        success: true,
        message: "Order cancelled successfully and product quantity updated",
      });
    } else {
...
const returnOrder = async (req, res) => {
  try {
    ...
    if (order) {
      ...
      res
        .status(404)
        .json({ success: false, message: "Error in return request" });
    }
  } catch (error) {
    console.error("Error returning order:", error);
    res.status(500).json({
      success: false,
      message: "Server error while Returning the order",
    });
  }
};
...
const getWishlist = async (req, res) => {
  try {
    ...
    const userData = await User.findById(user).populate({
      ...
      model: "Product",
      select:
        "productName productImages productOffer salePrice description regularPrice quantity _id",
    });
...
const cart = async (req, res) => {
  try {
    const user = req.session.user;
    const userData = await User.findOne({ _id: user });
    let userCart = await Cart.findOne({ userId: user }).populate(
      "items.productId"
    );
  ...
  try {
    ...
    if (userCart) {
      ...
      if (itemIndex > -1) {
        ...
        userCart.items[itemIndex].totalPrice = Math.round(
...
const checkCouponCode = async (req, res) => {
  try {
    ...
    if (totalAmount < coupon.minAmount || totalAmount > coupon.maxAmount) {
      return res.status(400).json({
        success: false,
        message: "This coupon is not applicable for this price range",
      });
    }
...
const downloadInvoice = async (req, res) => {
  ...
  try {
    ...
    const selectedAddress = await Address.findOne(
      { userId: req.session.user, "address._id": order.address },
      { "address.$": 1 }
    );
    ...
    const logoImg = fs.readFileSync(logoPath).toString("base64");
    ...
    doc.text(
      `Rs: ${(order.totalPrice.toFixed(2) - 50).toFixed(2)}`,
      170,
      finalY + 10
    );
...
function numberToWords(num) {
  ...
  const toWords = (n) => {
    ...
    for (let i = 0; i < thousands.length; i++) {
      ...
      if (n < divisor * 1000) {
        return (
          toWords(Math.floor(n / divisor)) +
          " " +
          thousands[i] +
          (n % divisor ? " " + toWords(n % divisor) : "")
        );
      }
    }
  };
