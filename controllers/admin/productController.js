...
const addProduct = async (req, res) => {
  upload(req, res, async (err) => {
    if (err) {
      ...
      return res
        .status(400)
        .json({ success: false, error: "File upload error" });
    }
    ...
    try {
      ...
      if (productOffer < 0 || productOffer > 100) {
        return res.status(400).json({
          success: false,
          error: "Offer must be less than 100% and greater than 0%",
          showModal: true,
        });
      }
      ...
      if (fileKeys.length < 3) {
        return res.status(400).json({
          success: false,
          error: "At least 3 product image is required",
          showModal: true,
        });
      }
  ...
  upload(req, res, async (err) => {
    ...
    try {
      ...
      if (quantity <= 0 || regularPrice <= 0) {
        return res
          .status(400)
          .json({ error: "Numbers Must be greater than 0" });
      }
...
const getOffers = async (req, res) => {
  try {
    ...
    const notOffer = await Product.find({
      productOffer: { $eq: 0 },
      isBlocked: false,
    });
    const offerBrands = await Category.find({
      categoryType: "brand",
      isListed: true,
      brandOffer: { $gt: 0 },
    });
    const notOfferBrands = await Category.find({
      categoryType: "brand",
      isListed: true,
      brandOffer: { $eq: 0 },
    });
...
const addOffer = async (req, res) => {
  try {
    ...
    if (percentage < 0 || percentage > 95) {
      return res
        .status(400)
        .json({ error: "Offer must be between 0% and 95%" });
    }
    ...
    res.json({
      success: true,
      message: `${offerType} offer added successfully`,
    });
  } catch (error) {
    res.status(500).json({ error: "Internal server error" });
  }
...
const addCoupon = async (req, res) => {
  try {
    ...
    if (existingCode) {
      return res
        .status(400)
        .json({ success: false, message: "This code is already taken" });
    }
